...
los procesos presentes en uno de los casos de estudio seleccionados.
...
lo tanto cualquier velocidad que se encuentre fuera del inervalo de
...
esto se ve como datos faltantes (DF). Para analizar el efecto de los
datos faltantes se aplicó una máscara de datos faltantes al campo de
\(V_{rs}'\) de tal manera que sean los mismo NAs presentes en el volumen
de datos de radar utilizados para que la distribución de Nas sea
...
amplitud de error lo cual sugiere que el algoritmo es robusto frente a
la presencia de errores aleatorios (no se muestra).
...
y 105 x 105 puntos de retícula y dominio 2 con una resolución de 4 x 4
km y 253 x 253 puntos de retícula, ambos centrados en la ubicación del
...
se ubique en 40 m para evitar errores en algunas variables de superficie
asociadas a la CLP.
...
parametrización analizada y el esquema de capa de superficie asociado a
...
usa la parametrización de capa de superficie proveniente del modelo MM5
que esta basada en la teoria de la semejanza de Monin-Obukohv.
...
Richardson se realiza para la capa de entremezcla entre la CLP y la
