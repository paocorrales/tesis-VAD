...
\begin{figure}[htbp]
\centering
\includegraphics{Tesis_files/figure-latex/topografia-1.pdf}
\caption{Topografía de la región en estudio en metros sobre el nivel del
mar. El punto negro indica la ubicación del Radar INTA Paraná, el punto
rojo indica la ubicación de la estación Paraná Aero. \label{topografia}}
\end{figure}
...
\tightlist
\item
  Viento moderado. En estas situaciones los datos de radar son más
  confiables y permiten el desarrollo de la capa límite estable nocturna
  (Gassmann y Mazzeo, 2001).
\item
  Cielos despejados. Para garantizar el calentamiento desde la
...
(por ejemplo Lim y Sun, 2010, y @Haase2004) con distinto grado de exito.
Una opción válida es el algoritmo de corrección de aliasing basado en
regiones similares disponible en la librería PyART ({\textbf{???}}),
este algoritmo busca regiones con velocidad radial similar y transforma
el rango de la variable hasta que todas las regiones fueron corregidas
de tal manera de obtener un campo continuo. El campo de velocidad radial
sin aliasing obtenido utilizando este algoritmo se muestra en la Figura
\ref{no-aliasing}.
...
Display, ({\textbf{???}})) y se muestra en la Figura \ref{vad}. Puede
observarse fácilmente que la velocidad radial media tiene un
...
radial o su gradiente con diferentes grados de complejidad que en la
literatura. Estas técnicas son también son llamdas VAD (o sus
derivaciones, Browning y Wexler (1968); Gao et~al. (2004); Xu et~al.
(2011)). Para esta tesis se desarrolló un algoritmo para el cálculo del
VAD siguiendo a Matejka y Srivastava (1991) pero incluyendo controles de
calidad específicos para asegurar la validez de los resultados.

\subsubsection{Desarrollo matemático}\label{desarrollo-matematico}
...
El algoritmo ajusta los datos de cada anillo realizando una regreción
lineal con la forma \(V_r = a_0 + a_1\cos \phi + b_1 \sin \phi\)
\textbf{Esto es una suma de series de fourier truncadas para i = 1}

Tambien se podría haber descompuesto \(V_r\) en series de Fourier de
orden superior para obtener otras características del viento. Pero por
ahora nos quedamos con esto.

\begin{figure}[htbp]
\centering
\includegraphics{Tesis_files/figure-latex/vad-1.pdf}
\caption{Velocidad radial (m/s) en función del azimut (grados) para un
rango y ańgulo de elevación fijos. En color se ajusta una función
sinusoidal a los datos.}
\end{figure}

\subsubsection{Control de calidad y
criterios}\label{control-de-calidad-y-criterios}
