...
gdata[ht > 0.02] %>%
  ...
  scale_x_datetime(name = "Tiempo (UTC)",
                   date_breaks = "4 hour",
                   date_labels = "%H:%M",
                   limits = c(as_datetime("2016-01-13 12:00:00"), NA),
                   expand = c(0,0)) +
  scale_y_continuous(name = "Altura (km)", limits = c(0,2),
                     expand = c(0,0)) +
...
```{r diff_spd, fig.cap="Diferencia entre las observaciones y cada simulación para la magnitud del viento (m/s) correspondiente al Caso 1. \\label{dif}", fig.height=0.3*text.height, fig.width=1.1*text.width}
gdata[ht > 0.02] %>%
  ggplot(aes(date_time, ht)) +
  geom_contour(aes(z = dif_spd, color = ..level..), binwidth = 1) +
  scale_color_divergent(name = "Diferencia en la\nmagnitud del viento", limits = c(-5, 5),
                      #breaks = seq(0, 18, 2), 
                      guide = guide_colorbar(order = 1)) +
  scale_x_datetime(name = "Tiempo (UTC)",
                   date_breaks = "4 hour",
                   date_labels = "%H",
                   limits = c(as_datetime("2016-01-13 12:00:00"), as_datetime("2016-01-14 11:00:00")),
                   expand = c(0,0)) +
  scale_y_continuous(name = "Altura (km)", limits = c(0,2),
                     expand = c(0,0)) +
...
gdata <- setDT(rbind(vad_caso1ysu, vad_caso1myj, vad_caso1acm)) %>%
  ...
  .[date_time %between% c(as_datetime("2016-01-13 12:00:00"), as_datetime("2016-01-14 12:00:00"))] %>%
...
gdata[, caso := factor(caso,
                       ...
                       ordered = T)] %>%
  .[, hora := hour(date_time)] %>%
  .[minute(date_time) == 00 & day(date_time) == 14 & hour(date_time) %in% c(08, 06) & ht < 2.5 &
      !is.na(spd_smooth)] %>%
...
gdata[ht %in% c(0.3, 0.5, 1.0) & !is.na(spd_smooth)] %>%
  ...
  scale_y_continuous(name = "v (m/s)") +
  coord_equal() +
  facet_wrap(~ht, labeller = labeller(ht = c("0.3" = "0.3 km",
                                             "0.5" = "0.5 km",
                                             "1" = "1.0 km"))) +
  theme_minimal() +
  theme(legend.position="bottom", legend.box = "horizontal", legend.margin=margin(t=-0.3, r=0, b=0, l=0, unit="cm"))
