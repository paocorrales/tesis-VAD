...
mapa.rios <- setDT(fortify(rnaturalearth::ne_download(scale = 10, type = 'rivers_lake_centerlines', category = 'physical', destdir = "seudocache/")))
...
```{r perfil-rmse, fig.cap="Velocidad del viento (m/s) en función de la altura para los distintos ángulos de elevación (puntos) y perfil final obtenido luego del primedio pesado (linea) calculados con VAD. \\label{perfil-rmse}", fig.width=0.75*text.width, fig.align='center'}
...
ggplot(vad, aes(ht, spd)) +
  ...
  annotate(geom = "rug", x = seq(0.1, 1.5, 0.1), color = "#20968b") +
  coord_flip() + 
  theme_minimal()
...
Como los datos de sondeo llegan hasta los 30 km de altura, hay información disponible para interpolar los datos de $V-r$ para todos los ángulos de elevación y para cualquier rango. Sin embargo esto da muchos más datos de los disponibles normalmente. Es de esperar que el VAD resultante sea muy similar al sondeo inicial pero esta primera validación también permite verificar que la transformación de $V$ a $V_r$ es correcta. 
...
geop[date %in% c(as_datetime("2016-01-14 00:00:00 UTC"), as_datetime("2016-01-14 12:00:00 UTC"))] %>% 
  ...
  theme_minimal() +theme(panel.ontop = T, panel.grid = element_line(linetype = 3))
...
gdata[date_time %between% as.POSIXct(c("2016-01-14 00:00:00", "2016-01-14 09:40:00"), tz = "UTC"),]  
  
gdata[caso == "Obs", .(ht, date_time, obs = spd_smooth)] %>% 
  .[gdata[caso != "Obs", .(ht, date_time, caso, spd_smooth)],
    on = c("ht", "date_time")] %>% 
  .[, .(bias = bias.f(spd_smooth, obs)), by = .(caso, ht)] %>% 
  ggplot(aes(ht, bias)) +
  geom_line(aes(color = caso)) + 
...
gdata[caso == "Obs", .(ht, date_time, obs = spd_smooth)] %>% 
  .[gdata[caso != "Obs", .(ht, date_time, caso, spd_smooth)],
    on = c("ht", "date_time")] %>% 
  .[, .(bias = bias.f(spd_smooth, obs)), by = .(caso, date_time)] %>% 
  .[day(date_time) == 14] %>% 
  ggplot(aes(date_time, bias)) +
  geom_line(aes(color = caso)) +
  scale_color_viridis(discrete = T)
...
```{r}
err <- gdata[day(date_time) == 14] %>% 
  .[caso == "Obs", .(ht, date_time, obs = spd_smooth)] %>% 
  .[gdata[caso != "Obs", .(ht, date_time, caso, spd_smooth)],
    on = c("ht", "date_time")] %>% 
  .[, .(bias = bias.f(spd_smooth, obs),
        rmse = rmse.f(spd_smooth, obs)), by = .(caso)]

bias <- err[, bias]
err2 <- gdata[day(date_time) == 14] %>%   
  .[caso == "Obs", .(ht, date_time, obs = spd_smooth)] %>% 
  .[gdata[caso != "Obs", .(ht, date_time, caso, spd_smooth)],
    on = c("ht", "date_time")] %>% 
  .[, .(rmsens = rmsens.f(spd_smooth, obs, bias)), by = .(caso)]

error <- cbind(err, err2)
error[,4] <- NULL



err <- gdata[day(date_time) == 14] %>% 
  .[caso == "Obs", .(ht, date_time, obs = di)] %>% 
  .[gdata[caso != "Obs", .(ht, date_time, caso, di)],
    on = c("ht", "date_time")] %>% 
  .[, .(bias = bias.f(di, obs),
        rmse = rmse.f(di, obs)), by = .(caso)]
...
err2 <- gdata[day(date_time) == 14] %>%   
  ...
  .[gdata[caso != "Obs", .(ht, date_time, caso, di)],
    on = c("ht", "date_time")] %>% 
  .[, .(rmsens = rmsens.f(di, obs, bias)), by = .(caso)]

err <- cbind(err, err2)
err[,c(1,4)] <- NULL
error <- cbind(error, err)
kable(error, digits = 4, caption = "E")
```
