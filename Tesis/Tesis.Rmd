...
```{r validacion-perfiles, fig.cap="Viento medio (m/s) en función de la altura a partir del sondeo, y las distintas pruevas de validación a la izquierda y el detalle ampliado del máximo en niveles bajos \\label{validacion-perfiles}", out.extra=" ", out.width="0.95\\textwidth", fig.show="h"}
...
```{r campo-caso1, fig.cap="Magnitud del viento y dirección correspondiente al Caso 1 (14/01/2016) observados por el radar y procesados con VAD \\label{campo-caso1}", fig.subcap=c("Magnitud del viento. \\label{caso1-spd}", "Dirección. \\label{caso1-dir}"), out.width="0.98\\textwidth", fig.height=0.4*text.height, fig.ncol=1}
...
```{r campo-caso2, fig.cap="Magnitud del viento y dirección correspondiente al Caso 2 (21/01/2016) observados por el radar y procesados con VAD \\label{campo-caso2}", fig.subcap=c("Magnitud del viento. \\label{caso2-spd}", "Dirección. \\label{caso2-dir}"), out.width="0.98\\textwidth", fig.height=0.4*text.height, fig.ncol=1}
...
```{r campo-caso3, fig.cap="Magnitud del viento y dirección correspondiente al Caso 3 (23/01/2016) observados por el radar y procesados con VAD \\label{campo-caso3}", fig.subcap=c("Magnitud del viento. \\label{caso3-spd}", "Dirección. \\label{caso3-dir}"), out.width="0.98\\textwidth", fig.height=0.4*text.height, fig.ncol=1}
...
```{r perfiles, fig.cap="Perfil de viento observado por el radar a las 0600 y las 1700 UTC y el $rmse_2$ calculado en sombreado para cada caso. El valor en superficie corresponde a lo observado por la estación meteorógica. \\label{perfiles-horarios}", out.width="0.95\\textwidth", fig.align='center'}
...
ri_vad[hour(hora) %between% c(0, 12)] %>% 
  ...
  geom_line(aes(color = caso)) +
  scale_color_viridis(name = "Casos", discrete = T) +
  scale_y_continuous(name = "u_max/z_max (s-1)") +
  scale_x_datetime(name = NULL, limits = lim) +
  theme_minimal()
...
ri_vad[hour(hora) %between% c(0, 12)] %>% 
  ...
  scale_y_continuous(name = "Richardson") +
...
```{r hodografa-horaria, fig.cap="Variación del vector del viento en función de la altura entre 100 y 3000 metros, para cada caso de estudio observada por el radar a las 0600 UTC. El triángulo marca el nivel inferior (z = 100 m) \\label{hodografa-h}", fig.align='center', out.width="0.75\\textwidth"}
...
vad_20160114[, caso := "Obs"]
```


```{r vad-modelo-spd, fig.cap="Magnitud del viento observada por el radar y simulada utilizando distintas parametrizaciones para el Caso 1 (14/01/2016) y posteriormente procesados con VAD \\label{modelo-spd}", out.width="0.98\\textwidth", fig.align='center'}
...
```{r vad-modelo-dir, fig.cap="Dirección del viento observada por el radar y simulada utilizando distintas parametrizaciones para el Caso 1 (14/01/2016) y posteriormente procesados con VAD. \\label{modelo-dir}", out.width="0.98\\textwidth", fig.align='center'}
...
```{r diferencia, fig.cap="diferencia \\label{dif}", fig.subcap=c("Magnitud del viento \\label{dif-spd}", "Dirección del viento \\label{dif-dir}"), fig.ncol=2}
...
Sin embargo se observa que esto no ocurre y que existen regiones donde el signo de L es más 40% de las veces distinto a la moda del dominio. Esto puede estar relacionado la variación en la altura del terreno o a la presencia del río y zonas con suelos saturados donde los flujos verticales de calor ($f_{\theta}$) tengan un comportamiento contrario al suelo seco.


```{r L-espacial, fig.cap="Porcentaje de veces que el valor local de L (a) y el flujo vertical de calor (b) tiene signo distinto a la moda para todo el periodo analizado, se muestran los valores mayores al 5% (colores) y topografía del dominio (contornos). Simulación con la parametrización YSU. \\label{L-esp}", fig.subcap=c("a \\label{L}", "b \\label{hfx}"), out.width="0.48\\textwidth", fig.ncol=2}
...
```{r L-parm, fig.cap="Longitud de Monin Obukhov (m) en función del tiempo y para cada parametrización. \\label{L-param}", out.width="0.98\\textwidth", fig.align='center'}
ggplot(all.pbl, aes(date, L)) +
  ...
  # ggplot(all.pbl[param == "MYJ"], aes(date, pblh)) +
  # geom_point(aes(color = regimen)) +
  # #facet_wrap(~param) +
