...
L <- caso.YSU[lev == -25, La := L*mode(L), by = .(date)] %>% 
  .[lev == -25, .(mean = mean(La < 0)), by = .(lon, lat)] %>% 
  .[mean > 0.05, ] %>% 
  .[, var := "L"]
...
ggplot(all.k[lev < 1500 & kh.u > 0], aes(date, lev)) +
  geom_contour(aes(z = kh.u, color = ..level..)) +
...
all.k[, `:=`(kh.u_norm = kh.u/(ust*0.4*pblh),
           ...
           lev_norm = lev/pblh),
    by = .(param, regimen)] %>% 
  .[, lapply(.SD, function(x) approx(lev_norm, x, xout = seq(0, 1, by = 0.1))$y), 
    by = .(param, regimen), .SDcols = c("lev_norm", "kh.u_norm", "km.u_norm")] %>% 
...
hL <- group_by(all.k, param, regimen) %>% 
  summarise(h_norm = mean(pblh, na.rm = T), L_norm = mean(L, na.rm = T)) %>% 
  .[, hL := h_norm/L_norm]
```
